...
@SuppressWarnings ({"NestedMethodCall", "DuplicateStringLiteralInspection", "MethodCallInLoopCondition", "StatementWithEmptyBody", "ChainedMethodCall", "BooleanVariableAlwaysNegated", "unused", "RedundantFieldInitialization"})
...
//                System.out.print("O moves: ");
    ...
    private static boolean xWon(StringBuilder board) {
        ...
        line1 = board.substring(positions[1], positions[3] + 1).equals("X X X");
        line2 = board.substring(positions[4], positions[6] + 1).equals("X X X");
        line3 = board.substring(positions[7], positions[9] + 1).equals("X X X");
        row1 = board.substring(positions[1], positions[1] + 1).equals("X") &&
                board.substring(positions[4], positions[6] + 1).equals("X") &&
                board.substring(positions[7], positions[9] + 1).equals("X");
        row2 = board.substring(positions[2], positions[2] + 1).equals("X") &&
                board.substring(positions[5], positions[5] + 1).equals("X") &&
                board.substring(positions[8], positions[8] + 1).equals("X");
        row3 = board.substring(positions[3], positions[3] + 1).equals("X") &&
    ...
    private static boolean oWon(StringBuilder board) {
        boolean line1, line2, line3, row1, row2, row3, diag1, diag2;
        line1 = board.substring(positions[1], positions[3] + 1).equals("O O O");
        line2 = board.substring(positions[4], positions[6] + 1).equals("O O O");
        line3 = board.substring(positions[7], positions[9] + 1).equals("O O O");
        row1 = board.substring(positions[1], positions[1] + 1).equals("O") &&
                board.substring(positions[4], positions[4] + 1).equals("O") &&
                board.substring(positions[7], positions[7] + 1).equals("O");
        row2 = board.substring(positions[2], positions[2] + 1).equals("O") &&
                board.substring(positions[5], positions[5] + 1).equals("O") &&
                board.substring(positions[8], positions[8] + 1).equals("O");
        row3 = board.substring(positions[3], positions[3] + 1).equals("O") &&
